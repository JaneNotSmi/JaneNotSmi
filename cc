...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('Mobile Kaitan New Version') then
        makefolder('Mobile Hub Kaitan New Version')
    end
    if not _G.Dis and not isfile('Mobile Kaitan New Version/'..Name..'.json') then
        writefile('Mobile Kaitan New Version/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('Mobile Kaitan New Version') and isfile('Mobile Kaitan New Version/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('Mobile Kaitan New Version/'..Name..'.json'))
...
game.StarterGui:SetCore("SendNotification", {
	Title = "Bẻo Phen", 
	Text = "Mobile Hub On topppp",
	Icon = "http://www.roblox.com/asset/?id=9610159123",
	Duration = 8000
})

local win = library:Window("Mobile",[[Hub]],[[Version : Premium Kaitan  ]],"9606070311",Enum.KeyCode.RightControl)
